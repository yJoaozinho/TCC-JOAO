...
export default function Post({ petId, nome, username, descricao }) {
    ...
    useEffect(() => {
        ...
        async function fetchPets() {
            ...
            try {
                const response = await fetch(`http://localhost:2306/animal/${idDoPet}`, {
                    method: 'GET',
                    headers: {
    ...
    return (
        <div className={Styles.post}>
            <div className={Styles.postHeader}>
                ...
                <div className={Styles.raiPaLa}><button className={Styles.petButton} onClick={() => router.push(`/perfilPet/${idDoPet}`)}>@{pet.nome}</button></div>
            </div>
